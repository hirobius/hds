/**
 * Tests for HdsMultiSelector — trigger label, open, and toggling options.
...
import { HdsMultiSelector, type HdsMultiSelectorOption } from './hds-multi-selector';
...
const OPTIONS: HdsMultiSelectorOption[] = [
  { value: 'a', label: 'Apple' },
...
function Example({ start = [] }: { start?: string[] } = {}) {
  ...
  return <HdsMultiSelector options={OPTIONS} value={value} onChange={setValue} />;
}

describe('HdsMultiSelector', () => {
  ...
  it('calls onChange with the option added when toggled on', () => {
    ...
    render(<HdsMultiSelector options={OPTIONS} value={[]} onChange={onChange} />);
    fireEvent.click(screen.getByRole('button'));
  ...
  it('calls onChange with the option removed when toggled off', () => {
    const onChange = vi.fn();
    render(<HdsMultiSelector options={OPTIONS} value={['a', 'b']} onChange={onChange} />);
    fireEvent.click(screen.getByRole('button'));
