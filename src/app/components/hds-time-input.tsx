/**
 * HdsTimeInput — token-skinned time field on a native <input type="time">.
 * @category Inputs
 * @tier primitive
 * @public
 *
...
type HdsTimeInputVariantProps = VariantProps<typeof hdsTimeInputVariants>;

/** @public */
export interface HdsTimeInputProps
  extends
    Omit<React.InputHTMLAttributes<HTMLInputElement>, 'type' | 'size'>,
    HdsTimeInputVariantProps {}

// ── Component ──────────────────────────────────────────────────────────────────

/** A native time field skinned with HDS input chrome. Value is a `HH:mm` string. */
export const HdsTimeInput = React.forwardRef<HTMLInputElement, HdsTimeInputProps>(
  function HdsTimeInput({ className, size, ...props }, ref) {
    return (
      <input
        ref={ref}
        type="time"
...
/** @internal — CVA variant helper; compose via HdsTimeInput props instead. */
