...
export const Panel: Story = {
  args: {
    ...
    children: (
      <Text variant="body">
        This content is revealed when the disclosure is expanded. It uses the panel variant for
...
export const OpenByDefault: Story = {
  args: {
    ...
    variant: 'panel',
    children: (
      <Text variant="body">This disclosure starts open. It can still be toggled closed.</Text>
    ),
  },
};
...
function ControlledRender() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
      ...
      <Disclosure label="Controlled disclosure" open={open} onOpenChange={setOpen}>
        <Text variant="body">State is owned by the parent component above.</Text>
      </Disclosure>
    </div>
  );
}
  ...
  render: () => (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', maxWidth: '480px' }}>
      <Disclosure label="What are design tokens?">
        <Text variant="body">
          Design tokens are named design decisions stored as key-value pairs, enabling consistent
          styling across platforms.
        </Text>
      </Disclosure>
      <Disclosure label="How does HDS handle theming?">
        <Text variant="body">
          HDS uses CSS custom properties resolved from the token system. Dark mode is handled via
          [data-theme] on the root element.
        </Text>
      </Disclosure>
      <Disclosure label="What is Ladle vs Storybook?">
        <Text variant="body">
          Storybook 8 was selected for HDS (over Ladle and Histoire) for its larger ecosystem,
          first-class a11y addon, and Chromatic integration.
        </Text>
      </Disclosure>
    </div>
  ),
