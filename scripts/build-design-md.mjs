...
export function buildColors(raw) {
  ...
  lines.push(
    `- **Feedback — Error** (\`${String(red).toUpperCase()}\`): destructive confirms, error banners, validation failures. \`semantic.color.feedback.error\`.`,
  );
  lines.push(
    `- **Feedback — Success** (\`${String(green).toUpperCase()}\`): positive confirmations, completed states. \`semantic.color.feedback.success\`.`,
  );
  lines.push(
    `- **Feedback — Warning** (\`${String(amber).toUpperCase()}\`): cautions, recoverable issues. \`semantic.color.feedback.warning\`.`,
  );
  lines.push(
    `- **Feedback — Info** (\`${String(info).toUpperCase()}\`): neutral announcements and inline guidance. \`semantic.color.feedback.info\`.`,
  );
  lines.push('');
  lines.push(
...
export function buildTypography(raw) {
  ...
  const sizeMap = Object.fromEntries(
    Object.keys(sizes)
      ...
      .map((k) => [k, resolveRef(sizes[k].$value, raw)]),
  );
  const weightMap = Object.fromEntries(
    Object.keys(weights)
      .filter((k) => !k.startsWith('$'))
      .map((k) => [k, resolveRef(weights[k].$value, raw)]),
  );
...
export function buildSpacing(raw) {
  ...
  const lines = [];
  lines.push(
    `**Base unit: ${base}.** All spacing snaps to the primitive scale; never introduce half-steps or arbitrary px values.`,
  );
  lines.push('');
...
export function buildRadius(raw) {
  ...
  lines.push(
    row(
      ...
      'Buttons, inputs, badges, alerts, disclosures, segmented control items',
    ),
  );
  lines.push(
    row(
  ...
  lines.push(
    row(
      ...
      'Pills, avatars, indicator dots, any intentionally circular form',
    ),
  );
  lines.push(
    row(
      ...
      `\`${zero}\``,
      '`primitive.radius.0`',
      'Outer canvas / substrate boundaries only — never on everyday UI controls',
    ),
  );
  return lines.join('\n');
}
...
export function buildMotion(raw) {
  ...
  for (const { path, value } of walk(durations)) {
    const key = path.at(-1);
    durLines.push(
      row(`\`primitive.duration.${key}\``, `\`${dim(value)}\``, durationUse[key] ?? ''),
    );
  }
...
export function buildComponents(raw, manifest) {
  ...
  const specs = [
    ...
    },
    {
      ...
      notes:
        'Border-driven treatment; no filled background by default. Focus uses `semantic.color.border.accent` plus a 2px outline offset. Error swaps to `component.input.borderError`.',
    },
    {
      name: 'Cards',
      component: 'Card',
    ...
    },
    {
      ...
      notes:
        'Single-line status markers. Feedback colors come from `semantic.color.feedback.*`. Never used as decorative chrome.',
    },
    {
      name: 'Alerts',
      component: 'Alert',
    ...
    },
    {
      ...
      notes:
        'Accordion primitive. Expansion uses `semantic.motion.productive`; no spring bounce. Dividers follow `semantic.color.border.subtle`.',
    },
    {
      name: 'Toggles',
      component: 'HdsToggle',
      ...
      notes:
        'Accent-filled track in the on state; neutral track otherwise. Track + thumb transitions share `semantic.motion.productive`.',
    },
    {
...
export function buildMeta(raw) {
  ...
  // No generation timestamp: a clock-based stamp churned DESIGN.md on every
  // `pnpm tokens` run. Git history records when the doc changed; the line only
  // changes now when the token count actually does.
  return [
    '---',
    '',
    `> Generated from \`hirobius.tokens.json\` (${tokenCount} tokens) and \`public/hds-manifest.json\` by \`scripts/build-design-md.mjs\`.`,
