 ...
 *
 * The output is a PROPOSAL (hds#235's disposition table), not a verdict. It
 * exists so that decision can be made against counts and names in one sitting.
 * Ratify by editing `figma/disposition.json`'s `overrides` — a name listed
 * there wins over the inferred class, permanently and with a reason.
 *
 * Usage:
...
if (write) {
  ...
  writeFileSync(
    ...
    `${JSON.stringify(
      {
        $comment:
          'PROPOSAL for hds#235. Generated by pnpm figma:disposition --write. `class` decides whether a component is expected to carry an @figma node: only `library` is. To ratify a different answer for one component, add it to `overrides` with a reason — an override wins over the inferred class and survives regeneration.',
        classes: CLASSES,
        generatedAt: new Date().toISOString().slice(0, 10),
        overrides: existing.overrides ?? {},
        summary: result.summary,
        components: result.components.map(({ inferred, overridden, overrideReason, ...rest }) => ({
          ...rest,
...
console.log(
  `\nThis is a proposal, not a verdict — ratify or redline it in hds#235, then` +
    `\nrecord exceptions in figma/disposition.json's \`overrides\`.\n`,
);
