...
export function buildSemanticColor(raw) {
  ...
  for (const { path, value, desc, ext } of walk(col)) {
    ...
    const note = desc ?? '';
    const lightVal = typeof value === 'string' && value.startsWith('{') ? value : `\`${value}\``;
    const darkVal = typeof dark === 'string' && dark.startsWith('{') ? dark : `\`${dark}\``;
...
export function buildSpacing(raw) {
  ...
  for (const { path, value, desc } of walk(space)) {
    ...
    const val = typeof value === 'object' ? `${value.value}${value.unit}` : String(value);
...
export function buildSize(raw) {
  ...
  lines.push(
    row(
      '`primitive.size.interactive.min`',
      `\`${resolveRef(size.interactive?.min?.$value ?? '', raw)}\``,
      'Compact touch target / hit-area width',
    ),
  );
  lines.push('');
  lines.push(
...
export function buildBrandIdentity(raw) {
  ...
  lines.push(
    row('Action radius', `\`${action}\` (\`semantic.radius.action\`) for interactive controls`),
  );
  lines.push(row('Card corners', `\`${radius8}\` (\`primitive.radius.8\`)`));
  lines.push(
    row('Motion philosophy', 'Depth via interaction (BulgeCard parallax), not drop shadows'),
  );
  return lines.join('\n');
}
...
export function buildTypography(raw) {
  ...
  const lhMap = Object.fromEntries(
    Object.keys(lhs)
      .filter((k) => !k.startsWith('$'))
      .map((k) => [k, resolveRef(lhs[k].$value, raw)]),
  );
  const lsMap = Object.fromEntries(
    Object.keys(lss)
      .filter((k) => !k.startsWith('$'))
      .map((k) => [k, resolveRef(lss[k].$value, raw)]),
  );
  ...
  const lines = [
    header('Style', 'Size (max)', 'Weight', 'Line Height', 'Letter Spacing', 'Description'),
  ];
...
export function buildRadius(raw) {
  ...
  for (const { path, value } of walk(radius)) {
    ...
    const val =
      typeof value === 'object' && 'value' in value && 'unit' in value
...
export function buildMotion(raw) {
  ...
  for (const { path, value, desc } of walk(easings)) {
    ...
    const val =
      value && typeof value === 'object' && value.type === 'spring'
  ...
  for (const { path, value, desc } of walk(motions)) {
    ...
    const easingText =
      easing && typeof easing === 'object' && easing.type === 'spring'
