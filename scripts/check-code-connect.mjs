...
export function renderPreviewReport({ summary, previews }) {
  const lines = [
    ...
    '',
    `templates: ${summary.templates} · mapped: ${summary.mapped.length ? summary.mapped.join(', ') : 'none'} · unmapped: ${summary.unmapped.length ? summary.unmapped.join(', ') : 'none'}`,
    `exempt public cva modules: ${
      Object.entries(summary.exempt)
        .map(([kind, count]) => `${kind} ${count}`)
        .join(' · ') || 'none'
    }`,
    `property combinations rendered: ${summary.combinations}`,
  ];
