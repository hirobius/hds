 ...
 *   node scripts/check-sync-map.mjs --write      # (re)write docs/sync-map.json
 *   node scripts/check-sync-map.mjs --check      # fail if docs/sync-map.json is stale
 *
...
// Prettier collapses short primitive arrays (storyFiles: [...], gaps: [...])
// onto one line, which plain JSON.stringify does not — format through the
// same helper other generators use (scripts/lib/write-generated.mjs) so
// --write produces the byte-identical bytes lint-staged's Prettier pass
// would produce anyway, and --check compares against that, not against raw
// JSON.stringify output.
const canonicalSyncMap = await formatGenerated(
  OUT_JSON,
  `${JSON.stringify(
    {
      $comment:
...
if (WRITE) {
  writeFileSync(OUT_JSON, canonicalSyncMap);
  console.log(`✓ wrote ${path.relative(ROOT, OUT_JSON)}`);
}

if (CHECK) {
  if (!existsSync(OUT_JSON)) {
    console.error(`✗ ${path.relative(ROOT, OUT_JSON)} is missing`);
    console.error('  fix: node scripts/check-sync-map.mjs --write');
    process.exit(1);
  }
  const onDisk = readFileSync(OUT_JSON, 'utf8');
  if (onDisk !== canonicalSyncMap) {
    let onDiskParsed = null;
    try {
      onDiskParsed = JSON.parse(onDisk);
    } catch {
      // fall through — report as unparseable below
    }
    console.error(`✗ ${path.relative(ROOT, OUT_JSON)} is out of date with its source records`);
    if (onDiskParsed?.rows && Array.isArray(onDiskParsed.rows)) {
      const onDiskByName = new Map(onDiskParsed.rows.map((r) => [r.name, r]));
      const freshByName = new Map(map.rows.map((r) => [r.name, r]));
      const changed = [];
      for (const [name, freshRow] of freshByName) {
        const oldRow = onDiskByName.get(name);
        if (!oldRow || JSON.stringify(oldRow) !== JSON.stringify(freshRow)) changed.push(name);
      }
      for (const name of onDiskByName.keys()) {
        if (!freshByName.has(name)) changed.push(name);
      }
      if (changed.length > 0) {
        console.error(`  ${changed.length} row(s) disagree: ${changed.sort().join(', ')}`);
      }
    } else {
      console.error('  (committed file does not parse as the expected shape)');
    }
    console.error('  fix: node scripts/check-sync-map.mjs --write, then commit the diff');
    process.exit(1);
  }
  console.log(`✓ ${path.relative(ROOT, OUT_JSON)} matches its source records`);
  process.exit(0);
}
