...
describe('draftCaseStudy', () => {
  it('returns a record with all canonical sections + slug + rawNotes', () => {
    const r = draftCaseStudy({ slug: 'client-a', text: 'just rambling' });
    expect(r.slug).toBe('client-a');
  ...
  it('extracts headings (## Context, ## Problem, …) when present', () => {
    const text = [
      '## Context',
      'Small business, solo founder, manual processes.',
      '',
      '## Problem',
      'Spending 6h/wk on inbox triage.',
      '',
      '## Approach',
      'Built classifier + mail rules.',
    ].join('\n');
    const r = draftCaseStudy({ slug: 'client-a', text });
