...
- **Generated artifacts are generated, not committed** — Figma variable exports (`hirobius.figma-variables*.json`) and the Figma model (`figma/model.json`) are deterministic outputs of `hirobius.tokens.json`; gitignored, regenerated via `pnpm figma-variables` / `pnpm figma:model`. The exporter is pinned by a golden snapshot of a fixture graph instead.
