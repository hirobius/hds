...
export default [
  ...
  // src/app/components/*.figma.ts are generated by
  // scripts/generate-code-connect.mjs and run inside Figma's template runtime,
  // which provides the `figma` module (typed by tsconfig.figma.json). It is not
  // resolvable from node_modules, so only that specifier is exempted.
  {
    files: ['src/**/*.figma.ts'],
    rules: {
      'import-x/no-unresolved': ['error', { ignore: ['^virtual:', '^figma$'] }],
    },
  },
