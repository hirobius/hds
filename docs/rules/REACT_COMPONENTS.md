...
4. **Variant contract:** Any component with structural/semantic/size/density
   variance is built with `cva` on the fixed `variant`/`tone`/`size`/`density`
   axes — see `docs/architecture/variant-contract.md` for the full contract
