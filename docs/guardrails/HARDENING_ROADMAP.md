...
- `.github/CODEOWNERS` even for a solo repo. Documents path → reviewer mapping. Forces explicit thought when paths cross client/agency boundaries (e.g. a `clients/<slug>/legal/` path requires the client's sign-off, even though the client has no GH account).
